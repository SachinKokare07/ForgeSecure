function DeviceTable({ logs }) {
  return (
    <div className="relative flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-950/60 shadow-[0_20px_80px_rgba(2,6,23,0.35)] max-h-[550px]">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="mb-1 flex items-center gap-2">
              ...
              <p className="text-[9px] font-semibold uppercase tracking-[0.3em] text-cyan-300">Industrial Telemetry</p>
            </div>
        ...
        {logs.length ? (
          <table className="w-full table-fixed text-left text-sm">
            <thead className="sticky top-0 z-10 bg-linear-to-r from-slate-950/95 to-slate-900/90 text-[11px] font-bold uppercase tracking-[0.15em] text-slate-300 backdrop-blur-sm">
              <tr className="border-b border-white/10">
                ...
                <th className="w-[12%] px-2 py-3 text-center sm:px-4">Traffic</th>
                <th className="w-[10%] px-2 py-3 text-center sm:px-3">CPU</th>
                <th className="w-[10%] px-2 py-3 text-center sm:px-3">Temp</th>
                <th className="w-[12%] px-2 py-3 text-center sm:px-4">Status</th>
                <th className="w-[12%] px-2 py-3 text-center sm:px-4">Severity</th>
                <th className="w-[16%] px-3 py-3 text-right sm:px-4">Time</th>
              </tr>
            </thead>
              ...
              {logs.map((log, idx) => (
                ...
                >
                  ...
                  <td className="px-2 py-2.5 text-center sm:px-4">
                    <span className={`inline-block px-2 py-1 rounded text-[10px] font-semibold whitespace-nowrap ${formatTrafficLevel(log.traffic)}`}>
                      {log.traffic}M
                    </span>
                  </td>
                  <td className="px-2 py-2.5 text-center sm:px-3">
                    <span className={`inline-block px-1.5 py-1 rounded text-[10px] font-semibold ${formatCpuLevel(log.cpu)}`}>
                      {log.cpu}%
                    </span>
                  </td>
                  <td className="px-2 py-2.5 text-center sm:px-3">
                    <span className={`inline-block px-1.5 py-1 rounded text-[10px] font-semibold ${formatTempLevel(log.temperature)}`}>
                      {log.temperature}°
                    </span>
                  </td>
                  <td className="px-2 py-2.5 text-center sm:px-4">
          ...
          <div className="flex items-center justify-center px-4 py-12 text-center">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Waiting for telemetry data...</p>
            </div>
          </div>
        )}
      </div>
...
function formatTrafficLevel(traffic) {
  const num = Number(traffic) || 0
  if (num > 3000) return 'bg-red-500/20 text-red-100 border border-red-500/30'
  if (num > 1000) return 'bg-amber-500/20 text-amber-100 border border-amber-500/30'
  return 'bg-cyan-500/15 text-cyan-100 border border-cyan-500/25'
}

function formatCpuLevel(cpu) {
  const num = Number(cpu) || 0
  if (num > 80) return 'bg-red-500/20 text-red-100 border border-red-500/30'
  if (num > 60) return 'bg-amber-500/20 text-amber-100 border border-amber-500/30'
  return 'bg-emerald-500/15 text-emerald-100 border border-emerald-500/25'
}

function formatTempLevel(temperature) {
  const num = Number(temperature) || 0
  if (num > 75) return 'bg-red-500/20 text-red-100 border border-red-500/30'
  if (num > 60) return 'bg-amber-500/20 text-amber-100 border border-amber-500/30'
  return 'bg-emerald-500/15 text-emerald-100 border border-emerald-500/25'
}
